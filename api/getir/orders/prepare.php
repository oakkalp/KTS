<?php
/**
 * GetirYemek API - Sipariş Hazırlama
 * GetirYemek siparişini hazır olarak işaretler
 */
...
try {
    ...
    $stmt = $db->query("
        ...
        WHERE s.id = ? AND s.mekan_id = ? AND s.source = 'getir' AND s.status = 'accepted'
    ", [$siparis_id, $mekan_id]);
    
    $siparis = $stmt->fetch();
    
    if (!$siparis) {
        throw new Exception('Sipariş bulunamadı veya onaylanmamış');
    }
    ...
    // GetirYemek API'sine sipariş hazırlama isteği gönder
    $getir_token = getGetirToken();
    
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, GETIR_API_BASE_URL . '/food-orders/' . $siparis['getir_order_id'] . '/prepare');
    ...
    // Log kaydet
    writeLog("GetirYemek sipariş hazırlandı: {$siparis['order_number']} (Getir ID: {$siparis['getir_order_id']})", 'INFO', 'getir.log');
    
    // Başarılı yanıt
    jsonResponse([
        'success' => true,
        'message' => 'Sipariş başarıyla hazır olarak işaretlendi',
        'data' => [
            'order_id' => $siparis_id,
    ...
    writeLog("GetirYemek sipariş hazırlama hatası: " . $e->getMessage(), 'ERROR', 'getir.log');
    
    jsonResponse([
        'success' => false,
        'error' => [
            'code' => 'INTERNAL_ERROR',
            'message' => 'Sipariş hazırlanırken hata oluştu: ' . $e->getMessage()
        ]
    ], 500);
}
