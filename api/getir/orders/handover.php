<?php
/**
 * GetirYemek API - Sipariş Teslim (Getir Getirsin)
 * GetirYemek siparişini Getir kuryesine teslim eder
 */
...
try {
    ...
    $stmt = $db->query("
        ...
        WHERE s.id = ? AND s.mekan_id = ? AND s.source = 'getir' AND s.status = 'ready' AND s.delivery_type = 1
    ", [$siparis_id, $mekan_id]);
    
    $siparis = $stmt->fetch();
    
    if (!$siparis) {
        throw new Exception('Sipariş bulunamadı, hazır değil veya Getir Getirsin siparişi değil');
    }
    ...
    // GetirYemek API'sine sipariş teslim isteği gönder
    $getir_token = getGetirToken();
    
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, GETIR_API_BASE_URL . '/food-orders/' . $siparis['getir_order_id'] . '/handover');
    ...
    // Log kaydet
    writeLog("GetirYemek sipariş Getir kuryesine teslim edildi: {$siparis['order_number']} (Getir ID: {$siparis['getir_order_id']})", 'INFO', 'getir.log');
    
    // Başarılı yanıt
    jsonResponse([
        'success' => true,
        'message' => 'Sipariş başarıyla Getir kuryesine teslim edildi',
        'data' => [
            'order_id' => $siparis_id,
    ...
    writeLog("GetirYemek sipariş teslim hatası: " . $e->getMessage(), 'ERROR', 'getir.log');
    
    jsonResponse([
        'success' => false,
        'error' => [
            'code' => 'INTERNAL_ERROR',
            'message' => 'Sipariş teslim edilirken hata oluştu: ' . $e->getMessage()
        ]
    ], 500);
}
