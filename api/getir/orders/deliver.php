<?php
/**
 * GetirYemek API - Sipariş Teslim (Restoran Getirsin)
 * GetirYemek siparişini müşteriye teslim eder
 */
...
try {
    ...
    $stmt = $db->query("
        ...
        WHERE s.id = ? AND s.mekan_id = ? AND s.source = 'getir' AND s.status = 'ready' AND s.delivery_type = 2
    ", [$siparis_id, $mekan_id]);
    
    $siparis = $stmt->fetch();
    
    if (!$siparis) {
        throw new Exception('Sipariş bulunamadı, hazır değil veya Restoran Getirsin siparişi değil');
    }
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, GETIR_API_BASE_URL . '/food-orders/' . $siparis['getir_order_id'] . '/deliver');
    ...
    // Log kaydet
    writeLog("GetirYemek sipariş teslim edildi: {$siparis['order_number']} (Getir ID: {$siparis['getir_order_id']})", 'INFO', 'getir.log');
