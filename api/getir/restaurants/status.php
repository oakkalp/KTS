<?php
/**
 * GetirYemek API - Restoran Durum Yönetimi
 * Restoran açma/kapama işlemleri
 */
...
try {
    ...
    $action = $input['action']; // 'open' veya 'close'
    ...
    // GetirYemek API'sine restoran durum değişikliği isteği gönder
    $getir_token = getGetirToken();
    
    $endpoint = ($action === 'open') ? '/restaurants/status/open' : '/restaurants/status/close';
    $request_data = [];
    ...
    // Mekan durumunu güncelle
    $is_active = ($action === 'open') ? 1 : 0;
    ...
    $db->query("
        ...
        SET is_active = ?,
            ...
            updated_at = NOW()
        WHERE id = ?
    ", [$is_active, $status, $mekan_id]);
    
    // Log kaydet
    $action_text = ($action === 'open') ? 'açıldı' : 'kapatıldı';
    writeLog("GetirYemek restoran {$action_text}: {$mekan['mekan_name']} (Getir ID: {$mekan['getir_restaurant_id']})", 'INFO', 'getir.log');
    
    // Başarılı yanıt
    jsonResponse([
        'success' => true,
        'message' => "Restoran başarıyla {$action_text}",
        'data' => [
            'mekan_id' => $mekan_id,
    ...
    jsonResponse([
        ...
        'error' => [
            'code' => 'INTERNAL_ERROR',
            'message' => 'Restoran durumu değiştirilirken hata oluştu: ' . $e->getMessage()
        ]
    ], 500);
}
