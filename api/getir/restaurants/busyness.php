<?php
/**
 * GetirYemek API - Restoran Yoğunluk Yönetimi
 * Restoran yoğunluğunu yönetme
 */
...
try {
    ...
    if (!isset($input['isBusy'])) {
        throw new Exception('isBusy parametresi eksik');
    }
    
    $db = getDB();
    $mekan_id = $user['type_id'];
    $is_busy = (bool)$input['isBusy'];
    $busyness_difference_duration = $input['busynessDifferenceDuration'] ?? 15; // 15, 30, 45 dakika
    ...
    // GetirYemek API'sine yoğunluk durumu değişikliği isteği gönder
    $getir_token = getGetirToken();
    
    $request_data = [
        'isBusy' => $is_busy,
        'busynessDifferenceDuration' => $busyness_difference_duration
    ];
    
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, GETIR_API_BASE_URL . '/restaurants/delivery-duration/busyness');
    ...
    // Mekan yoğunluk durumunu güncelle
    $db->query("
        UPDATE mekanlar 
        SET is_busy = ?,
            busyness_duration = ?,
            updated_at = NOW()
        WHERE id = ?
    ", [$is_busy, $busyness_difference_duration, $mekan_id]);
    
    // Log kaydet
    $busyness_text = $is_busy ? 'yoğun' : 'normal';
    writeLog("GetirYemek restoran yoğunluk durumu değiştirildi: {$mekan['mekan_name']} - {$busyness_text} (+{$busyness_difference_duration} dk)", 'INFO', 'getir.log');
    
    // Başarılı yanıt
    jsonResponse([
        'success' => true,
        'message' => "Restoran yoğunluk durumu başarıyla güncellendi",
        'data' => [
            'mekan_id' => $mekan_id,
            'mekan_name' => $mekan['mekan_name'],
            'getir_restaurant_id' => $mekan['getir_restaurant_id'],
            'is_busy' => $is_busy,
            'busyness_duration' => $busyness_difference_duration
        ]
    ]);
    
} catch (Exception $e) {
    writeLog("GetirYemek yoğunluk durumu değişikliği hatası: " . $e->getMessage(), 'ERROR', 'getir.log');
    
    jsonResponse([
        'success' => false,
        'error' => [
            'code' => 'INTERNAL_ERROR',
            'message' => 'Yoğunluk durumu değiştirilirken hata oluştu: ' . $e->getMessage()
        ]
    ], 500);
}
